export default function CategoryBox(props) {
    let category = props.singleCategoryData
    let className = "categoryBox " + category.level;

    // Only displays the details if the index of the box matches the state of the parent element
    // if (props.isActive) {
    //     return (
    //         <div className={className}>
    //             <h3>{category.name}</h3>
    //             <h4>{category.level}</h4>
    //             <p>{category.description}</p>
    //             <h5>Current Score: {category.value}</h5>
    //             {/* displays threshold for next tier, or current percentile if there is no next tier */}
    //             {category.nextThreshold ? (
    //                 <h5>Next Tier: {category.nextThreshold}</h5>
    //             ) : (
    //                 // Why on god's green earth is this the correct way to round numbers to 2 sig. figs.....
    //                 <h5>Current Percentile: {Number(Math.round((category.percentile * 100) + 'e2') + 'e-2') + "%"}</h5>
    //             )}
    //         </div>
    //     );
    // } else {
        return (
            <div className={className} onClick={props.handleClick}>
                <h3>{category.name}</h3>
                {/* <h4>{category.level}</h4> */}
            </div>
        );
    }
// }
